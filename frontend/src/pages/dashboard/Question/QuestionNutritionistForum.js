...
import { fetchNutritionistForumQuestion } from '../../../redux/slices/questionSlice';
...
export default function QuestionNutritionistForum() {
  ...
  useEffect(() => {
    dispatch(fetchNutritionistForumQuestion());
  }, [dispatch]);
          ...
          {(!questions.length ? [...Array(12)] : questions).map((question, index) =>
            question ? (
              <Grid key={question._id} item xs={12} sm={6} md={6}>
                <QuestionCard question={question} index={index} Urole={'nutritionist'} />
              </Grid>
            ) : (
