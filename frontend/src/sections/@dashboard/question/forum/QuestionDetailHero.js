...
QuestionDetailHero.propTypes = {
  question: PropTypes.object.isRequired,
};

export default function QuestionDetailHero({ question }) {
  const isDesktop = useResponsive('up', 'sm');

  return (
    <Box sx={{ position: 'relative' }}>
      <TitleStyle>Lets provide clients with some insight by answering their questions</TitleStyle>

      <FooterStyle>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Avatar
            alt={question.client.firstName}
            src={
          ...
          <Box sx={{ ml: 2 }}>
            <Typography variant="subtitle1" sx={{ color: 'common.white' }}>
              {`${question.client.firstName} ${question.client.lastName}`}
            </Typography>
      ...
      <OverlayStyle />
      <Image alt="post cover" src={'/illustrations/coverQuestionDetail.png'} ratio="21/9" />
    </Box>
  );
}
